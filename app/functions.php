...
function get_transacciones() {
  $stmt = 'SELECT * FROM tipo_transaccion ORDER BY id DESC';
  return ($rows = query_db($stmt)) ? $rows : false;
}

//creamos una funcion para traer los inmuebles pero solo los inmuebles que haya registrado el usuario de la cuenta actual iniciada
function get_inmuebles($id_user) {
  //creamos una variale para sacar el id del usuario alcual
  $id_user = cur_user()['id'];
  //para despues seleccionar un inmueble que este registrado por el usuario actual
  $stmt = "SELECT * FROM stock WHERE id_usuario = $id_user  ORDER BY id DESC";
  return ($rows = query_db($stmt)) ? $rows : false;
}
...
function get_clientes() {
  ...
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(c.id) AS total FROM clientes c';
  $total_records = query_db($stmt)[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
...
function get_clientes_by_user($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(c.id) AS total FROM clientes c WHERE c.id_usuario=:id_usuario';
  ...
  $stmt = 'SELECT c.* FROM clientes c WHERE c.id_usuario=:id_usuario ORDER BY c.id DESC '.$limit;
...
// Cargar todos los clientes de acuerdo a sus estatus
function get_clientes_by_estatus($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(c.id) AS total FROM clientes c WHERE c.id_usuario=:id_usuario and estatus IN ("", "Envio de ficha informativa", "Envio de contrato", "Negociacion con prospecto")';
  $total_records     = query_db($stmt , ['id_usuario' => $id_usuario])[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
  ...
  // Ahora solo seleccionaremos a todos los clientes que tengan como estatus el envio de informacion, de contrato y de negociacion de prospectos
  // corresponda al $id_usuario pasado
  $stmt = 'SELECT c.* FROM clientes c WHERE c.id_usuario=:id_usuario and estatus IN ("", "Envio de ficha informativa", "Envio de contrato", "Negociacion con prospecto")ORDER BY c.id DESC '.$limit;

  return ($rows = query_db($stmt , ['id_usuario' => $id_usuario])) ? [$rows,$pagination] : false;
}


// Cargar todos los clientes de acuerdo a  estatus = cierre de contrato
function get_clientes_by_contrato($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(c.id) AS total FROM clientes c WHERE c.id_usuario=:id_usuario and estatus IN ("Cierre de contrato")
  AND factura IN ("pagada") AND contrato IN ("pagada")';
  $total_records     = query_db($stmt , ['id_usuario' => $id_usuario])[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
  ...
  // Ahora solo seleccionaremos a todos los clientes que tengan como estatus el envio de informacion, de contrato y de negociacion de prospectos
  // corresponda al $id_usuario pasado
  $stmt = 'SELECT c.* FROM clientes c WHERE c.id_usuario=:id_usuario and estatus IN ("Cierre de contrato") AND factura IN ("pagada") AND contrato IN ("pagada") ORDER BY c.id DESC '.$limit;
...
// Cargar todos los clientes de acuerdo a pago = no pagado
function get_clientes_by_pago($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(id) AS total FROM clientes  WHERE id_usuario=:id_usuario AND estatus IN ("Cierre de contrato") AND factura IN ("", "no pagada") AND contrato IN ("pagada", "no pagada")';
  $total_records     = query_db($stmt , ['id_usuario' => $id_usuario])[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
  ...
  // Ahora solo seleccionaremos a todos los clientes que tengan como estatus el envio de informacion, de contrato y de negociacion de prospectos
  // corresponda al $id_usuario pasado
  $stmt = 'SELECT * FROM clientes  WHERE id_usuario=:id_usuario AND estatus IN ("Cierre de contrato") AND factura IN ("no pagada") AND contrato IN ("no pagada") ORDER BY id DESC '.$limit;
...
// Cargar todos los clientes de acuerdo a pago = no pagado
function get_clientes_by_conf($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(id) AS total FROM clientes  WHERE id_usuario=:id_usuario AND estatus IN ("Cierre de contrato") AND factura IN ("") AND contrato IN ("")';
  $total_records     = query_db($stmt , ['id_usuario' => $id_usuario])[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
...
// Cargar todos los clientes de acuerdo a pago = no pagado
function get_transaccion($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(id) AS total FROM tipo_transaccion  WHERE id_usuario=:id_usuario ';
  $total_records     = query_db($stmt , ['id_usuario' => $id_usuario])[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
  ...
  // Ahora solo seleccionaremos a todos los clientes que tengan como estatus el envio de informacion, de contrato y de negociacion de prospectos
  // corresponda al $id_usuario pasado
  $stmt = 'SELECT * FROM tipo_transaccion  WHERE id_usuario=:id_usuario ORDER BY id DESC '.$limit;
...
// Cargar todas por cuentas por pagar
function get_cuentas_by_pagar($id_usuario) {
  // Para paginación de registros
  $stmt              = 'SELECT COUNT(id) AS total FROM clientes WHERE id_usuario=:id_usuario AND estatus IN ("no pagada", "")';
  $total_records     = query_db($stmt , ['id_usuario' => $id_usuario])[0]['total'];
  $offset            = NULL;
  $pagination        = NULL;
  $limit             = NULL;
  ...
  // Quiero seleccionar TODAS las columnas de la tabla cuentas por pagar del usuario que 
  // corresponda al $id_usuario pasado
  $stmt = 'SELECT * FROM clientes WHERE id_usuario=:id_usuario AND estatus IN ("no pagada", "") ORDER BY id DESC '.$limit;

  return ($rows = query_db($stmt , ['id_usuario' => $id_usuario])) ? [$rows,$pagination] : false;
}

// siempre vamos a crear dos funciones de acuerdo a lo que registraremos o actualizaremos, ejemplo la funcion de arriba solo hara 
//la funcion de registrar y se puede identificar por que la peticion es que selleccione todo de la tabla cuentas_porpagar donde 
//el id_usuario sea == al id del usuario actual
//_-----------------
//y para actualizar hacemos lo contrario en vez que sea la peticion de que el id_usuario sea igual al id_usuario actual
// en esta ocacion sera solo el id del registro por que ese id lo traeremos con un get para validad y hacer la actualizacion

function get_cuentas_by_id($id) {
  $stmt = 
...
function get_pagos_by_id($id) {
  $stmt = 
  'SELECT *FROM informes_clientes WHERE id=:id  LIMIT 1';
  return ($rows = query_db($stmt , ['id' => $id])) ? $rows[0] : false;
}

function get_tipo_transaccion_by_id($id) {
  $stmt = 
  'SELECT *FROM tipo_transaccion WHERE id=:id  LIMIT 1';
  return ($rows = query_db($stmt , ['id' => $id])) ? $rows[0] : false;
}

function get_envios_by_id($id_venta) {
  $stmt = 
  'SELECT *FROM envios WHERE id_venta=:id_venta  LIMIT 1';
  return ($rows = query_db($stmt , ['id_venta' => $id_venta])) ? $rows[0] : false;
}
